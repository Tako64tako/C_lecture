#include <stdio.h>
#include <string.h>

#define MAX_CELLS (100 + 1) // セルの個数 \
                            // (cells[1]〜cells[100])の100個)
typedef struct assocList
{
    char name[20];
    int data;
    int next; // 次に処理すべきデータの添字番号
} alst;

alst cells[MAX_CELLS]; // セル領域のための配列変数
int cell_ptr;          //「次にどのセルを使うか」を示す変数

// ------------------------------------------------------------
void init_cells()
{          // セル領域の初期化
    int i; // (「全セル，使っていない」状態に)

    cell_ptr = 1;
    for (i = 1; i < MAX_CELLS; i++)
        cells[i].next = -1;
}

int cell_available()
{ // 空きセルがあるときに真
    if (cell_ptr == -1)
        return 0; // (cell_ptr == -1 のときは
    return 1;     //  空きセルなし)
}

void release_cell(int id)
{                        // 添字番号idのセルを，未使用
    cells[id].next = -1; // 状態にする
    if (cell_ptr == -1)
        cell_ptr = id;
}

int get_cell()
{               // 空きセルの添字番号が cell_ptr に
    int i, ret; // 入っているはずなので，それを返す．
                // さらに，次の空きセルがあるかを
    if (cell_available())
    { // チェックして，cell_ptr に反映
        ret = cell_ptr;
        for (i = cell_ptr + 1; i < MAX_CELLS; i++)
        {
            if (cells[i].next == -1)
            {
                cell_ptr = i;
                return ret;
            }
        }
        for (i = 1; i < cell_ptr; i++)
        {
            if (cells[i].next == -1)
            {
                cell_ptr = i;
                return ret;
            }
        }
        cell_ptr = -1;
        return ret;
    }
    return 0;
}

// ------------------------------------------------------------
int add_elm(char name[], int data, int lst)
{
    int id; // リストlstの先頭にセルを加える

    if (cell_available())
    {                                 // セルが使えるなら，
        id = get_cell();              // セル (番号 id) をゲットして，
        strcpy(cells[id].name, name); // データと次のセル番号 (lst) を
        cells[id].data = data;        // セットする
        cells[id].next = lst;
        return id; // idを返す
    }
    return 0; // セルがないときはエラー (0)
}

int add_elm_tail(char name[], int data, int lst)
{
    int ret_lst, id;

    if (lst == 0)
        return add_elm(name, data, lst);
    if (cell_available())
    {
        ret_lst = lst;
        for (; cells[lst].next != 0; lst = cells[lst].next)
            ;
        id = get_cell();
        strcpy(cells[id].name, name);
        cells[id].data = data;
        cells[id].next = 0;
        cells[lst].next = id;
        return ret_lst;
    }
    return 0;
}

// ------------------------------------------------------------
void print_cell(alst st)
{   // alst型の要素 st を表示
    // ([キー：alice, データ：50] のように)
    printf("[名前:%s, データ:%d]\n",st.name,st.data);
}

void print_lst(int lst){
     // リスト lst の全要素を表示
    while(1){
        if(lst == 0) return;
        print_cell(cells[lst]);
        lst = cells[lst].next;
    }

}

// ------------------------------------------------------------
int find_elm(char name[], int lst)
{ // キー「name」でセルを検索
    while (1)
    {
        if (lst == 0)
            return 0;
        if (!strcmp(name, cells[lst].name))
            return lst;
        lst = cells[lst].next;
    }
}

alst id2alst(int id)
{                    // セルの番号(id)を与えると，
    return cells[id]; // その中身 (alst型の要素) を取り出す
}

// ------------------------------------------------------------
int setv(int lst, char name[], int data)
{
    int i;
    if ((i = find_elm(name, lst))){                         // すでに そのキーで
        cells[i].data = data; // 値がセットされて
        return lst;           // いたなら 更新処理
    }
    else{
        return add_elm_tail(name, data, lst);
    } // さもなくば lst の
} // 末尾にセルを追加

int valof(int lst, char name[]){
    int i;

    if ((i = find_elm(name, lst)))
        return id2alst(i).data;
    return 0;
}

int maxof(int lst)
{
    int max = cells[lst].data;
    while (1)
    {
        if (lst == 0)
            break;
        if (max < cells[lst].data)
        {
            max = cells[lst].data;
        }
        lst = cells[lst].next;
    }
    return max;
}

// int maxof(int lst){
//     int max=0;
//     for(int i=0;i <= MAX_CELLS; i++){
//         if (lst == 0)
//         break;
//         if(max<cells[lst].data){
//             max = cells[lst].data;
//         }
//         lst = cells[lst].next;
//     }
//     return max;
// }

int mincell(int lst){
    int min = 100;
    int tensu, ret=0;
    while (1){
        if (lst == 0)
            return ret;
        tensu = id2alst(lst).data;
        if (min > tensu){
            ret = lst;
            min = tensu;
        }lst = id2alst(lst).next;
    }
}

int del_elm(char name[], int lst)
{
    int prev = -1, lst_top = lst, ret;

    while (1)
    {
        if (lst == 0)
            return 0; // リストが空っぽなら0を返す
        if (!strcmp(name, cells[lst].name))
        {
            if (prev == -1)
            {                          // 先頭の要素を消す場合
                ret = cells[lst].next; // リンクを貼り直して
                release_cell(lst);     // セルを開放し，
                return ret;            // リストの先頭になるセルを返す
            }
            else
            {
                cells[prev].next = cells[lst].next; // 2番目以降の要素を
                release_cell(lst);                  // 消す場合も同様だが
                return lst_top;                     // 予めリストの先頭を
            }                                       // 覚えておいて返す．
        }
        prev = lst;            // 空リストでも削除対象でもない
        lst = cells[lst].next; // ときは，参照対象を次のセルへ
    }                          // 再設定．
}

// int selection_sort(int lst)
// {
//     if(lst == 0) return 0;
//     int tmp = add_elm_tail(cells[lst].name,cells[lst].data,0);
//     int ten = id2alst(lst).data ,min = 100;
//     lst = del_elm(cells[lst].name,cells[lst].data);
//     while (1){
//         add_elm_tail(cells[lst].name, cells[lst].data, tmp);
//         lst = del_elm(cells[lst].name, cells[lst].data);
//         }
//         tmp = id2alst(lst).next;
//         if(lst == 0) return tmp;
// }

int selection_sort(int lst){
    if(lst == 0) return 0;
    int min = mincell(lst);
    int tmp = add_elm_tail(id2alst(min).name,id2alst(min).data,0);
    lst = del_elm(id2alst(min).name,lst);
while(1){
    min = mincell(lst);
    add_elm_tail(id2alst(min).name, id2alst(min).data, tmp);
    lst = del_elm(id2alst(min).name, lst);
    if(lst == 0) return tmp;
}
}

// ------------------------------------------------------------
int main()
{
    int math, eng, math2;

    init_cells();

    math = setv(0, "alice", 80);
    setv(math, "bob", 90);
    setv(math, "dave", 72);
    // printf("%d",math);
    eng = setv(0, "alice", 73);
    setv(eng, "bob", 80);
    setv(eng, "dave", 950); // わざと間違えておく
    // printf("%d",eng);
    printf("math ----------\n");
    print_lst(math);
    printf("eng ----------\n");
    print_lst(eng); // daveの点数がおかしい

    setv(eng, "dave", 95); // 正しい点数で上書き
    printf("eng (corrected) ----------\n");
    print_lst(eng);

    printf("alice's data ---------\n");
    printf("alice's eng score = %d\n", valof(eng, "alice"));
    //printf("alice's math score = %d\n",valof(math,"alice"));
    printf("alice's total score = %d\n",valof(eng, "alice") + valof(math, "alice"));

    printf("max,min------------\n");
    printf("maxof math score = %d\n",maxof(math));
    printf("maxof eng score = %d\n",maxof(eng));

    printf("min cell on eng score :");
    print_cell(id2alst(mincell(eng)));
    //printf("%d\n",mincell(eng));

    printf("selection_sort------------\n");
    math2 = selection_sort(math);
    print_lst(math2);

    return 0;
}
