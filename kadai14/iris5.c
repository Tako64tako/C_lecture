...
int main()
{
    ...
    for (i = 0; i < FORLOOP; i++)
    {
        ...
        else if (strcmp(iris_cat[i].class, virginica) == 0)
        {
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (i == 1)
        {
            avg_se_len[i] = sum_se_len[i] / b;
            sqt_se_len[i] = sqrt(sumv_se_len[i] / b - avg_se_len[i] * avg_se_len[i]);
            avg_se_wid[i] = sum_se_wid[i] / b;
            sqt_se_wid[i] = sqrt(sumv_se_wid[i] / b - avg_se_wid[i] * avg_se_wid[i]);
            avg_pet_len[i] = sum_pet_len[i] / b;
            sqt_pet_len[i] = sqrt(sumv_pet_len[i] / b - avg_pet_len[i] * avg_pet_len[i]);
            avg_pet_wid[i] = sum_pet_wid[i] / b;
            sqt_pet_wid[i] = sqrt(sumv_pet_wid[i] / b - avg_pet_wid[i] * avg_pet_wid[i]);
        }
        if (i == 2)
        {
            avg_se_len[i] = sum_se_len[i] / c;
            sqt_se_len[i] = sqrt((sumv_se_len[i] / c) - (avg_se_len[i] * avg_se_len[i]));
            avg_se_wid[i] = sum_se_wid[i] / c;
            sqt_se_wid[i] = sqrt((sumv_se_wid[i] / c) - (avg_se_wid[i] * avg_se_wid[i]));
            avg_pet_len[i] = sum_pet_len[i] / c;
            sqt_pet_len[i] = sqrt(sumv_pet_len[i] / c - (avg_pet_len[i] * avg_pet_len[i]));
            avg_pet_wid[i] = sum_pet_wid[i] / c;
            sqt_pet_wid[i] = sqrt(sumv_pet_wid[i] / c - (avg_pet_wid[i] * avg_pet_wid[i]));
        }
    }
    for (i = 0; i < 3; i++)
    {
        ...
        {
            printf("Iris-virsinica(avg,sd)\n");
        }
        printf("  sepal_length:(%f,%f)\n", avg_se_len[i], sqt_se_len[i]);
        printf("  sepal_width:(%f,%f)\n", avg_se_wid[i], sqt_se_wid[i]);
        printf("  petal_length:(%f,%f)\n", avg_pet_len[i], sqt_pet_len[i]);
        printf("  petal_width:(%f,%f)\n", avg_pet_wid[i],sqt_pet_wid[i]);
    }
}