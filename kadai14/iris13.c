...
int main()
{
    ...
    int y[5], i, j, n = 0, k, hage[30], num, tmp, sum;
    double tru = 0, con = 0, per = 0, abc,nm,xyz,a;
    char setosa[20] = "Iris-setosa\n", virginica[20] = "Iris-virginica\n", versicolor[20] = "Iris-versicolor\n";
    int no[10][10];
    double suma[3], nin[10][10];
    ...
    for (k = 0; k < sum; k++)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            for (j = 0; j < num - n; j++)
            {
                iris_set[i] = sqrt(pow(array_learn[j].sepal_length - array_test[i].sepal_length, 2.0) +
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            {
                a  = (no[i][j] / suma[i]) * 100;
                nin[i][j] += (no[i][j] / suma[i]) * 100;
            }
        }
    }
    printf("\t\t\t５分割クロスバリデーション\n");
    printf("\t\t\t\t推定\n");
