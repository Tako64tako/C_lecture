...
int main()
{
    ...
    int y[5], i, j, n = 0, k, hage[30], num, tmp;
    ...
    if (n > num / 2)
    {
        printf("指定した値がデータの半分以上です\n分割できませんでした\n");
        return 0;
    }
    if (num % n != 0)
    {
        printf("データをうまく分割できませんでした\n割り切れる値を入力してください\n");
        return 0;
    }
    printf("テストデータを何番目にするか入力してください\n");
    scanf("%d",&k);
    if (k >= n)
    {
        printf("分割数より大きい値は無効です\n");
        return 0;
    }
    n = num / n;
    ...
    printf("learndata----\n");
    for(i=0;i < num - n ;i++){
        printf("%s",array_learn[i].class);
    }
    printf("\ntestdata----\n\n");
    for(i=0;i<n;i++){
        printf("%s",array_test[i].class);
    }
        return 0;
    }